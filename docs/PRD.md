...
- 兌換流程（商店選擇 → OTP → QR 券）。
- 券夾。
- 設定（資安中心：本機資料檢視、生物鎖、一鍵清除、開源連結、隱私說明）。
...
- **輸入/功能**：檢視本機儲存了什麼、開關生物辨識鎖、一鍵清除所有本機資料、開源 repo 連結、隱私說明、重看導覽。
- **驗收**：
  - Given 使用者，When 點「一鍵清除」，Then 清空 Keychain 與所有本機快取／cookie，回到初始狀態。
  - Given 進資安中心，When 檢視，Then 明列「儲存於 Keychain 的欄位清單」與「本 App 不含後端、不對外傳個資」聲明。
...
| M5 | 資安中心 + 開源交付 | 一鍵清除、README/威脅模型、CI 檢查 |
