...
- **上限**：4000 字元　**實際**：1133 字元（2026-09-06 重算：原 947 字元，新增「使用統計預設關閉」「不看廣告、不被追蹤」兩條並改寫既有三條後為 1133，仍遠低於上限）
...
・使用統計預設關閉。App 內建 Google Firebase 的匿名使用統計與當機回報，出廠是關的，要你自己到「我的資料」打開才會送；送出的只有「開了哪個畫面、哪一步失敗、有沒有當機」，不含身分證號、生日、手機號碼與任何健康數據。隨時可以關掉。
...
- **上限**：4000 字元　**實際**：284 字元（2026-09-06 重算：原 239 字元，新增匿名統計一行後為 284）
...
・匿名使用統計與當機回報（Google Firebase）預設關閉，要在「我的資料」自己打開才會送，不含個資與健康數據。
・程式碼以 MIT 授權開源。
```
...
2. **隱私權政策 URL**：必須是**獨立可直接開啟**的網址（不能只是 App 內頁面）。內容至少要寫清楚：
   ...
   - **匿名使用統計與當機回報**：使用 Google Firebase Analytics／Crashlytics，**預設關閉**、使用者自行開啟；送出什麼、送給誰（Google LLC，伺服器在美國）、怎麼關掉。
