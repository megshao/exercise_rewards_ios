...
- **上限**：170 字元　**實際**：93 字元（2026-09-06 重算：原 85 字元，加入「開發者」與「與健康數據」後為 93）

```
今天走到了嗎？打開就看得到步數。這是協助你參加「揮汗有禮」活動的非官方個人工具——登入、看任務、管理加碼券都在同一頁。開發者沒有伺服器，個資與健康數據只留在你的手機，程式碼全部開源可查。
```
...
【你的資料在哪裡】
・開發者沒有伺服器。沒有自建後端，收不到也看不到你的身分資料與健康數據。
・只連官方網站。App 自己發出的請求只會到 500.gov.tw，其他網域一律阻擋。
・只存這支手機。身分證號、出生日期、手機號碼加密保存在 iOS Keychain，不同步 iCloud、不寫入紀錄檔、不會提供給任何第三方。
・使用統計：先告知，同意後預設開啟。App 內建 Google Firebase 的匿名使用統計與當機回報。第一次打開 App 會先看到一張免責聲明，上面就明寫這件事，你按下同意它才啟動——在那之前 Firebase 一行程式碼都不會執行。同意之後預設是開的，隨時可以到「我的資料」關掉。送出的只有「開了哪個畫面、哪一步失敗、有沒有當機」，不含身分證號、生日、手機號碼與任何健康數據。
...
・不外傳。健康數據只在你的手機上顯示，不會上傳到任何地方，也不會進入使用統計——連「今天有沒有達標」都不會送出去。
...
・匿名使用統計與當機回報（Google Firebase）：首次啟動的免責聲明會先告知，你按下同意才會啟動，之後預設開啟、可隨時在「我的資料」關掉，不含個資與健康數據。
・程式碼以 MIT 授權開源。
```
