...
【你的資料在哪裡】
・開發者收不到你的資料。沒有自建後端，看不到也存不到你的身分資料。
・個資只送官方網站。帶著你登入狀態的連線只會到 500.gov.tw，其他網域一律阻擋。只有兩個唯讀例外：看截圖時官方網站指定的圖片網域，以及券夾找不到品項頁時從 GitHub 下載一份所有人共用的公開商品目錄備份檔——那次連線不送出任何資料。
...
・隨時刪光。「我的資料」頁按下「立即登出並清除本機資料」，就永久刪除。
