...
- The app's own HTTP client is restricted to that one domain. This is enforced **twice**:
  ...
  2. **Client-side allowlist** — `URLSessionHTTPClient` rejects any request whose host is not `500.gov.tw` or a subdomain (suffix-spoofing such as `evil500.gov.tw` is explicitly blocked) and throws `blockedEgress`. This is the client that holds the signed-in cookie jar.
  3. **One read-only destination outside that allowlist** — `VendorCatalogService` downloads a public static file, `https://megshao.github.io/exercise_rewards_ios/vendor-catalog.json`, over its own `ephemeral` session with no cookie storage and no credentials. It uploads nothing, the file is identical for every user, and it is fetched at most once per app session and only when the wallet cannot otherwise recover a redeemed voucher's catalogue-page path. It is deliberately kept off the main allowlist so the credential-bearing session can never reach a third-party host. Demo mode uses a stub that performs no request at all. Disclosed in section 3 of the privacy policy.
