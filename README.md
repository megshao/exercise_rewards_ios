...
- `App/`（SwiftUI，XcodeGen 產生 `.xcodeproj`）：UI、HealthKit 實作、送審示範模式（`DemoMode`）
  ...
  - `App/Sources/App/Telemetry.swift`：全 App 唯一的 Firebase Analytics／Crashlytics 出口，封閉列舉 + 四道閘門，預設關閉（詳見下方威脅模型的「遙測」列）
