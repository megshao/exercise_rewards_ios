...
/// 「當期依日期前進」這個回報的 bug。但示範資料裡**沒有**「已過期卻仍 `.open`」的期別
/// （過去的 5 期都已走完審核，當期依定義涵蓋今天），所以「過期就不給上傳鈕」那條分支
/// **在 UI 層做不出來**——除非替示範資料加一個測試專用的注入點，而那會踩到
/// `DemoMode.swift` 裡 `ScreenshotMode` 明訂的界線（啟動參數只能影響畫面呈現，
/// 不可以拿來改資料來源）。
///
/// 因此那個判斷被搬進 Kit 成為 `TaskPeriod.canUpload(now:)`，由 `UploadWindowTests`
/// 覆蓋整個「狀態 × 日期」矩陣；`TasksView` 這一側只剩「true 給按鈕、false 給一行字」，
/// 外加 `TasksView.swift` 檔尾那個把兩種樣子並排畫出來的 `#Preview`。
/// 這裡則負責另一半：當期那張卡片**確實**拿得到上傳鈕，而且只有它拿得到。
@MainActor
